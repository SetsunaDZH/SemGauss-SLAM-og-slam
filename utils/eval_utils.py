...
def eval(config, dataset, final_params, num_frames, eval_dir, sil_thres,
    ...
    # Use the actual dataset sample resolution/intrinsics so eval stays correct
    # for downscaled runs (e.g. desired_image_width/height overrides).
    sample_color, _, _, sample_intrinsics, _ = dataset[0]
    H = int(sample_color.shape[0])
    W = int(sample_color.shape[1])
    sample_intrinsics = sample_intrinsics[:3, :3]
    fx = float(sample_intrinsics[0, 0].item())
    fy = float(sample_intrinsics[1, 1].item())
    cx = float(sample_intrinsics[0, 2].item())
    cy = float(sample_intrinsics[1, 2].item())
